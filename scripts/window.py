...
# NOTE: the axis for pygame is x -->
#                             y |
#                               v
# Therefore, we need to replace x and y from the
# input given, for a good view experience
...
cube_points[0] = [[-1 + left_eye_offside], [-1], [1]]
cube_points[1] = [[1 + left_eye_offside],[-1],[1]]
cube_points[2] = [[1 + left_eye_offside],[1],[1]]
cube_points[3] = [[-1 + left_eye_offside],[1],[1]]
cube_points[4] = [[-1 + left_eye_offside],[-1],[-1]]
cube_points[5] = [[1 + left_eye_offside],[-1],[-1]]
cube_points[6] = [[1 + left_eye_offside],[1],[-1]]
cube_points[7] = [[-1 + left_eye_offside],[1],[-1]]
...
while True:
    ...
    for point in cube_points:
        # Translaction
        translated_point = [[point[0][0] - left_eye_offside], [point[1][0]], [point[2][0]] ]

        # Final Rotation Matrix is Rx*Ry*Rz
        rotate_x = multiply_m(rotation_x, translated_point)
        rotate_y = multiply_m(rotation_y, rotate_x)
        rotate_z = multiply_m(rotation_z, rotate_y)

        rotate_z = [[rotate_z[0][0] + left_eye_offside], [rotate_z[1][0]], [rotate_z[2][0]] ]
